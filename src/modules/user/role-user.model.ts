...
function initRoleModel(model: typeof RoleUser, tableName: string, role: Roles): void {
  model.init(
    {
      id: {
        type: DataTypes.UUID,
...
export class AdminRoleUser extends RoleUser {}
export class VendorRoleUser extends RoleUser {}

initRoleModel(AdminRoleUser, 'admin_users', Roles.ADMIN);
initRoleModel(VendorRoleUser, 'vendor_users', Roles.VENDOR);

export type RoleUserModel = ModelStatic<any>;

export function getRoleUserModel(role: Roles): RoleUserModel {
  if (role === Roles.ADMIN) return AdminRoleUser as RoleUserModel;
  if (role === Roles.VENDOR) return VendorRoleUser as RoleUserModel;
  return User as RoleUserModel;
}
